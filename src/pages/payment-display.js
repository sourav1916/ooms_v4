...
const ViewPayments = () => {
    ...
    const handlePaymentSuccess = (paymentData) => {
        ...
        if (dateRange) {
            ...
            fetchPaymentsData(from, to, currentPage, searchTerm);
        }
    };
    ...
    useEffect(() => {
        ...
        const toDisplay = formatDateForDisplay(lastDay);
        const fromAPI = formatDateForAPI(firstDay);
        const toAPI = formatDateForAPI(lastDay);

        setDateRange(`${fromDisplay} - ${toDisplay}`);
        setFromToDate(`From ${fromDisplay} to ${toDisplay}`);
        fetchPaymentsData(fromAPI, toAPI, 1, '');
    }, []);
    ...
    const fetchPaymentsData = async (fromDate, toDate, page = 1, search = '') => {
        setLoading(true);
        setError(null);
        ...
        try {
            ...
            setError('Network error: Failed to fetch payment data');
            setPayments([]);
            setTotalAmount(0);
        } finally {
            setLoading(false);
    ...
    const handlePageChange = (newPage) => {
        ...
        if (dateRange) {
            ...
            const from = convertToAPIDate(displayFrom);
            const to = convertToAPIDate(displayTo);
            fetchPaymentsData(from, to, newPage, searchTerm);
        }
    };
    ...
    const getActionLinks = (item) => {
        ...
        switch (partyType) {
            ...
            case 'agent':
                ...
                break;
            case 'capital':
                editLink = `/edit-payment-client-capital?redirect=${window.location.href}&payment_id=${item.transaction_id}`;
                break;
            default:
                editLink = '#';
                invoiceLink = '#';
    ...
    // Get party type display info for payment recipient (payment_to)
    const getPaymentToInfo = (item) => {
        const type = item.payment_to?.type || '';
        ...
        switch (type) {
            ...
            case 'capital':
                ...
                break;
            case 'vendor':
                displayName = details.name || 'Vendor';
                email = details.email || '';
                mobile = details.mobile || '';
    ...
    const getCreatorTypeInfo = (item) => {
        ...
        if (username === 'admin' || username.includes('admin')) {
            ...
            bgColor = 'bg-blue-100';
            textColor = 'text-blue-700';
        } else if (username.includes('partner')) {
            type = 'partner';
            bgColor = 'bg-purple-100';
            textColor = 'text-purple-700';
        }
