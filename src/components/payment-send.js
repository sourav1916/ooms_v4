...
const PaymentSend = ({
    ...
    // Auto-select when initialUsername passed – fetch and set selected client
    useEffect(() => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (parseFloat(formData.amount) > (selectedBank?.balance || 0)) {
        ...
        try {
            const payload = {
                ...
                party1_id: formData.bank_id,
                party1_type: "bank",
                party2_id: formData.client_username,
                party2_type: "client",
                remark: formData.remark || `Payment made to ${selectedClient?.name || selectedClient?.username}`,
                transaction_date: formData.payment_date
            };

            console.log('Payment send payload:', payload);
            ...
            if (response.data.success) {
                toast.success(response.data.message || 'Payment sent successfully');
                ...
                toast.error(response.data.message || 'Failed to send payment');
            }
        } catch (error) {
